...
//                sender.sendMessage(main.getMessages().get(32));
            ...
            if (sender.hasPermission("greq.ticket")) {
                if (sender instanceof Player) {
                    Player p = (Player) sender;
                    if (!main.getBlockedUntil().containsKey(p)) {
                        if (args.length > 0) {
                            String text = "";
...
//                        p.sendMessage(main.translateDatabaseVariables(main.getMessages().get(0), id));
                        ...
                        main.sendMessage(p, main.getMessages().get(48).replace("%date", new SimpleDateFormat(main.getMessages().get(30)).format(main.getBlockedUntil().get(p))), -1);
                        return true;
                    }
                } else {
//                    sender.sendMessage("You have to be a player to use this command.");
