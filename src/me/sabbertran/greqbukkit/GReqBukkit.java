...
public class GReqBukkit extends JavaPlugin {
    ...
    public void setupMessages() {
        ...
        temp.add("Successfully purged all tickets.");
    ...
    private String translateDatabaseVariables(String input, int id) {
        ...
        try {
            PreparedStatement pst = sqlhandler.getCurrentConnection().prepareStatement("SELECT * FROM greq_tickets WHERE id = ?");
            pst.setInt(1, id);
...
//                        p.sendMessage(translateDatabaseVariables(messages.get(7), id));
    ...
    public void sendTicketList(CommandSender p, int amount, int page) {
        try {
            PreparedStatement pst = sqlhandler.getCurrentConnection().prepareStatement("SELECT * FROM greq_tickets WHERE (status = '0' OR status = '1') ORDER BY id DESC LIMIT ?, ?");
            pst.setInt(1, (amount * page - amount));
            pst.setInt(2, (amount * page));
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                rs.last();
                int size = rs.getRow();
    ...
    public void unclaimTicket(Player p, int id) {
        try {
            ...
            if (rs.next()) {
                ...
                String status_extra = rs.getString("status_extra");
                if (status == 1) {
                    PreparedStatement pst1 = sqlhandler.getCurrentConnection().prepareStatement("UPDATE greq_tickets SET status = '0' WHERE id = ? AND status = '1' AND status_extra = ?");
                    pst1.setInt(1, id);
                    pst1.setString(2, p.getName());
                    pst1.execute();
...
//                    p.sendMessage(translateDatabaseVariables(messages.get(24), id).replace("%name", unclaimer));
    ...
    public void sendOwnTicketList(Player p, boolean closed) {
        String sql_cmd;
        if (!closed) {
            sql_cmd = "SELECT * FROM greq_tickets WHERE author = ? AND (status = '0' OR status = '1')";
        } else {
            sql_cmd = "SELECT * FROM greq_tickets WHERE author = ?";
        }
        try {
            PreparedStatement pst = sqlhandler.getCurrentConnection().prepareStatement(sql_cmd);
            pst.setString(1, p.getName());
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                rs.last();
...
//                p.sendMessage(messages.get(39));
    ...
    public boolean reopenTicket(int id) {
        try {
            PreparedStatement pst = sqlhandler.getCurrentConnection().prepareStatement("SELECT * FROM greq_tickets WHERE id = ? AND (status = '2' OR status = '3')");
            pst.setInt(1, id);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                PreparedStatement pst1 = sqlhandler.getCurrentConnection().prepareStatement("UPDATE greq_tickets SET status = 0, answer = NULL WHERE id = ?");
                pst1.setInt(1, id);
                pst1.execute();
                return true;
            }
    ...
    public boolean purgeTickets(int type) { //0 = all; 1 = closed
        String sql_cmd;
        if (type == 0) {
            sql_cmd = "DELETE FROM greq_tickets";
        } else if (type == 1) {
            sql_cmd = "DELETE FROM greq_tickets WHERE (status = '2' OR status = '3')";
        } else {
            return false;
        }
